...
```
# Data entry here having all data for query ----
```
...
# Here we have some problems to solve to improve our knowlwdge for PostgreSQL

 # Q-1 Fine top 1 or 3 outlets by cuisine type
```
select * from 
	(select 
		cuisine,
		restaurant_id,
		count(*) as no_order,
		dense_rank() over(partition by cuisine order by count(*) desc) as rank
	from orders
	group by 1,2
	)
where rank=1	
```
----------------------------------
# Q-2 Find daily new customer count from the launch date(everyday how many new customer we are aquiring)
```
with cte as(
select 
		customer_code,
...
select
	first_order_date,
	count(*) as no_new_customer
from cte
group by 1
```
---------------------------------------------
# Q-3 Find of all users who were acquired in Jan 2025 and only placed one order in Jan and did't placed any other order
select * from orders
```
select 
	customer_code,
	count(*) as no_of_order
	--extract(year from placed_at) as year,
	--extract(month from placed_at) as month
from orders
	where extract(year from placed_at)=2025 and extract(month from placed_at)=1 and
	customer_code not in(select distinct customer_code from orders
							where not(extract(year from placed_at)=2025 and extract(month from placed_at)=1))
group by 1
having count(*)=1
```
-----------------------------------------------------
# Q-4 List all customers with no order in last 7 days but were acquired one month ago with their first order on promo
--select current_date-interval '329 days'
select * from orders
```
with cte as(
	select 
		customer_code,
		min(placed_at::date) as first_order_date,
		max(placed_at::date) as last_order_date
	from orders
	group by 1)
select 
	cte.*,
	orders.promo_code_name
from cte 
join orders on cte.customer_code=orders.customer_code and
	cte.first_order_date=orders.placed_at::date
	where 
	last_order_date <  current_date-interval '336 days'
	and first_order_date < current_date-interval '366 days'
	and orders.promo_code_name is not null
```
---------------------------------------------------------------
/*
# Q-5 Growth team is planning to create a trigger that will target customers after their every third order with a personalized communication and they have asked you to create a query for this.
*/
select * from orders
```
with cte as(	
	...
	from orders)
select * from cte	
where order_no % 3=0 and placed_at::date='2025-03-31' --on specific date notification
```
---------------------------------------------------------------
/*
# Q-6 List customers who placed more than 1 order and all their orders on a promo only
*/
select * from orders
```
select 
...
-------------------------------------------------------------------
/*
# Q-7 what percentage of customers were originally acquired in Jan 2025.
(placed their first order without promo code )
*/
```
	with cte as
	(select *,
		dense_rank() over(partition by customer_code order by placed_at ) as rank
